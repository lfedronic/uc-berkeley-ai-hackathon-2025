...
export function layoutNodeToFlexLayout(layout: LayoutNode): IJsonModel {
  const convertNode = (node: LayoutNode): any => {
    if (node.type === 'tabset') {
      ...
      const activeIndex = node.tabs?.findIndex(tab => tab.id === node.activeTabId) || 0;
...
export function flexLayoutToLayoutNode(flexModel: IJsonModel): LayoutNode {
  const convertNode = (node: any): LayoutNode => {
    if (node.type === 'tabset') {
      const tabs: Tab[] = node.children?.map((child: any) => ({
        id: child.id,
        title: child.name,
 ...
 */
export function findNodeInLayout(layout: LayoutNode, id: string): LayoutNode | null {
 ...
 */
export function validateLayoutDepth(layout: LayoutNode, currentDepth = 0): boolean {
